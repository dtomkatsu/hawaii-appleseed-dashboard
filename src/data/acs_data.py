...
class ACSDataFetcher:
    ...
    def _setup_connection(self):
        """Set up the cenpy connection to the ACS API."""
        try:
            cenpy.set_sitekey(self.api_key, overwrite=True)
            self.acs = cenpy.products.ACS(year=self.year)
            logger.info(f"Successfully connected to ACS {self.year}")
        except Exception as e:
            logger.error(f"Error connecting to ACS API: {str(e)}")
            raise
    ...
    def get_acs_data(
        ...
        try:
            ...
            # Only add county parameter for non-legislative district levels
            if level in ['tract', 'block group', 'county']:
                query_params['county'] = county if county else '*'
            
            logger.info(f"Fetching ACS data for {len(variables)} variables at {level} level")
            
            # Fetch the data using cenpy
            if level == 'county':
                gdf = self.acs.from_place(
                    query_params,
                    place_type='county',
                    strict_within=False,
                    return_bounds=False,
