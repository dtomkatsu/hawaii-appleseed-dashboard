"""
Test script for the ACS data fetcher using direct Census API calls with 2023 ACS data
"""
import os
import sys
import logging
...
def test_state_house_districts(api_key=None):
    ...
    try:
        # Initialize the ACS data fetcher with 2023 data
        logger.info("Initializing ACSDataFetcher for 2023 ACS 5-year estimates...")
        fetcher = ACSDataFetcher(api_key=api_key, year=2023)
...
def test_tract_data(api_key=None):
    """Test fetching data for census tracts."""
    try:
        # Initialize the ACS data fetcher with 2023 data
        logger.info("Initializing ACSDataFetcher for 2023 ACS 5-year estimates...")
        fetcher = ACSDataFetcher(api_key=api_key, year=2023)
        ...
        # Fetch data for tracts
        logger.info("Fetching data for Hawaii census tracts...")
        tract_data = fetcher.get_acs_data(
            variables=variables,
            level='tract',
            state='15',  # Hawaii
            geometry=False
        ...
        if tract_data is not None and not tract_data.empty:
            # Display basic info
            logger.info(f"Retrieved data for {len(tract_data)} census tracts")
            logger.info("Columns in the data:")
            ...
            # Show first few rows of data
            logger.info("\nSample data (first 5 rows):")
            logger.info(tract_data.head().to_string())
            
            # Calculate poverty rate
            tract_data = fetcher.calculate_poverty_rate(tract_data)
            
            # Save to CSV for inspection
            output_file = fetcher.save_to_csv(tract_data, 'hawaii_tracts_acs_2023')
            logger.info(f"\nData saved to {output_file}")
            
            return tract_data
        else:
            logger.warning("No data was returned for census tracts")
            return None
            
    except Exception as e:
        logger.error(f"Error in test_tract_data: {str(e)}", exc_info=True)
        raise

if __name__ == "__main__":
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description='Test ACS data fetching with Census API (2023 ACS 5-year estimates)')
    parser.add_argument('--api-key', help='Census API key (if not provided, will check CENSUS_API_KEY environment variable)')
    parser.add_argument('--test', choices=['county', 'tract', 'house', 'all'], default='all',
    ...
    debug_log_file = logs_dir / "test_acs_2023_debug.log"
    ...
    try:
        ...
        if args.test in ['tract', 'all']:
            # Test tract data
            logger.info("\n=== Testing tract data ===")
            test_tract_data(api_key=api_key)
        
        if args.test in ['house', 'all']:
            # Test state house districts
